...
static Run execute(Api& api, uint32_t exponent, uint32_t device, bool cache) {
  ...
  _putenv_s("AEVUM_PFA_LEAD_BRIDGE", cache ? "1" : "0");
#else
  setenv("AEVUM_REG_LEAD_CACHE", cache ? "1" : "0", 1);
  setenv("AEVUM_PFA_LEAD_BRIDGE", cache ? "1" : "0", 1);
#endif
  constexpr const char* spec = "pfa9:1:512:9:512:202";
  ...
  if (run.transform != 4718592u) throw std::runtime_error("unexpected PFA9 FFT3161 transform size");
...
int main(int argc, char** argv) {
  try {
    ...
    std::cout << "PFA9 FFT3161 LEAD-BRIDGE DIFFERENTIAL TEST PASSED\n";
    return 0;
